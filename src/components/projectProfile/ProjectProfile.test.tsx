...
import Example from './RegistryRequest';
import i18n from '../../i18n';
import { I18nextProvider } from 'react-i18next';
import { initialState } from './duck/reducers';
...
it('Form exist in the component', () => {
  const { getByTestId } = render(<ExampleWithI18nProvider example={initialState} />);
  expect(getByTestId('form-test-id')).toBeTruthy();
});

it('Success alert is not shown at first in the component', () => {
  const { queryByTestId } = render(<ExampleWithI18nProvider example={initialState} />);
  expect(queryByTestId('success-alert-test-id')).toBeFalsy();
});

test('When an input is filled is value change', () => {
  const { container } = render(<ExampleWithI18nProvider example={initialState} />);
...
test('If the form is not filled, the success alert is not shown', () => {
  const { container, queryByTestId } = render(<ExampleWithI18nProvider example={initialState} />);
