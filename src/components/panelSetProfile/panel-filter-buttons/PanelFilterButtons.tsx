...
interface IProps {
  ...
  setFilter: (newPanelFilter: PanelFilter) => void;
}
...
const PanelFilterButtons = (props: IProps) => {
  ...
  const handlePanel = useCallback(() => {
    history.push(routes.PATH_TAB_PANEL_DIAGNOSTIC);
  }, [history]);
    ...
    <>
      <Box display="flex" flexDirection="column">
        <Box display="flex" flexDirection="row" padding={2} alignSelf="end" alignItems="center" justifyContent="flex-end" style={{ width: '100%' }}>
          ...
          {!props.panelSetData.deletionDate && <GaiaButton variant="outlined" icon={<Add />} text={t('commons.buttons.new')} onClick={handlePanel} style={{ marginLeft: 20 }} />}
        </Box>
        <Popover
          id="simple-popover"
          open={expanded}
        ...
        >
          <Grid container justify="space-between" alignItems="center" spacing={1}>
            ...
            <Grid item xs={12}>
              <Typography style={{ paddingTop: 5, paddingBottom: 5 }}>{t('commons.fields.ascendingPanels')}</Typography>
              <FilterCheckTrueFalseField
                value={filter.ascendingPanels}
                labelTrue={t('commons.fields.yesNoField.yes')}
                labelFalse={t('commons.fields.yesNoField.no')}
                field="ascendingPanels"
                onChange={handleFilter}
              />
            </Grid>
            <Grid item xs={12}>
              <Typography style={{ paddingTop: 5, paddingBottom: 5 }}>{t('commons.fields.descendingPanels')}</Typography>
              <FilterCheckTrueFalseField
                value={filter.descendingPanels}
                labelTrue={t('commons.fields.yesNoField.yes')}
                labelFalse={t('commons.fields.yesNoField.no')}
                field="descendingPanels"
                onChange={handleFilter}
              />
            </Grid>
            <Grid item xs={6}>
...
export default PanelFilterButtons;
