...
class MmpCancerClient {
  ...
  public static async getDrugsByDrugset<Drug>(id: string | undefined, searchText: string | undefined, dateFrom: Date | undefined, abortSignal: AbortSignal): Promise<Drug> {
    const queryParams = this.setQueryParams(searchText, dateFrom);
  ...
  private static readonly setQueryParams = (searchText: string | undefined, dateFrom: Date | undefined) => {
    if (!searchText && !dateFrom) {
      return undefined;
    } else if (searchText && !dateFrom) {
